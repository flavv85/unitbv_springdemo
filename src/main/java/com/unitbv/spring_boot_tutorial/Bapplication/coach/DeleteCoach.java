...
import com.unitbv.spring_boot_tutorial.Ddomain.exceptions.BusinessException;
...
public class DeleteCoach {
    ...
    public void delete(String id) {
        Coach coach = coaches.getCoachById(id).orElseThrow(
                () -> new BusinessException(String.format("Coach with id %s was not found", id)));
        coaches.delete(coach);
    }

    public void deleteById(String coachId) {
        coaches.getCoachById(coachId).orElseThrow(
                () -> new BusinessException(String.format("Coach with id %s was not found", coachId)));
